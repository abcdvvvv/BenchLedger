...
describe("Dimension Selector hierarchy", () => {
  ...
  it("shows up to four selected values, summarizes larger selections, and disables a single-value selector", () => {
    ...
    base.dimensionSelection.fixedValueSelections = [{ dimension: Kind, valueKeys: ["git"], rememberedValueKeys: ["git"], options: [{ key: "git", label: "git", configurationCount: 2 }] }, { dimension: four, valueKeys: fourOptions.map((option) => option.key), rememberedValueKeys: fourOptions.map((option) => option.key), options: fourOptions }, { dimension: five, valueKeys: fiveOptions.map((option) => option.key), rememberedValueKeys: fiveOptions.map((option) => option.key), options: fiveOptions }];
