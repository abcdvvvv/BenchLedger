...
import type { ResolvedDimensionSelection } from "./dimension-selector";

export type BenchmarkViewBenchmarkOption = { value: string; path: string[]; label: string; };
type BenchmarkViewFilterState = { yAxis: string; branch: string; timeStartValue: number | null; timeEndValue: number | null; displayStrategy: DisplayStrategy; configurationKeys: readonly string[]; };
export function resolveBenchmarkViewFilter(catalog: BenchmarkViewCatalog, state: BenchmarkViewFilterState): BenchmarkViewFilterState {
  ...
  return { ...state, yAxis: effectiveYAxis, branch: effectiveBranch };
}

export function buildBenchmarkOptions(keys: readonly string[], benchmarksByKey: ReadonlyMap<string, BenchmarkDefinition>): BenchmarkViewBenchmarkOption[] { return keys.map((key) => benchmarksByKey.get(key) ?? { key, path: [], label: key }).map((benchmark) => ({ value: benchmark.key, label: benchmark.label, path: benchmark.path })).sort((left, right) => left.path.join("\0").localeCompare(right.path.join("\0")) || left.label.localeCompare(right.label) || left.value.localeCompare(right.value)); }
...
type BenchmarkDimensionPointAggregate = { point_key: string; point_label: string; benchmark_key: string; metric_name: string; statistic: string; unit: string; value: number; better: BenchmarkTrendAggregateRow["better"]; run_count: number; representative_run_id: string; };

export function buildDimensionPointAggregates(aggregates: readonly BenchmarkTrendAggregateRow[], runsById: ReadonlyMap<string, BenchmarkRun>, dimensionSelection: ResolvedDimensionSelection): BenchmarkDimensionPointAggregate[] {
  ...
  for (const aggregate of aggregates) {
    ...
    const pointIndex = pointOrder.get(pointKey); if (pointIndex === undefined) continue; const point = dimensionSelection.points[pointIndex];
...
export function buildTrendRowsByBenchmark(aggregates: readonly BenchmarkTrendAggregateRow[], runsById: ReadonlyMap<string, BenchmarkRun>, selectedBenchmarkKeys: readonly string[], dimensionSelection: ResolvedDimensionSelection): Map<string, TrendPlotRow[]> {
  ...
  for (const aggregate of buildDimensionPointAggregates(aggregates, runsById, dimensionSelection)) {
    ...
    if (!parseDate(run.code_date)) continue;
    ...
    const entry: TrendPlotRow = { unit: aggregate.unit, value: aggregate.value, code_date: run.code_date, measured_at: run.measured_at, run_axis_label: runAxisLabel(run), run_identity_title: `${aggregateLabel}<br>${runIdentityTitle(run, "<br>")}`, x_label: aggregate.point_label };
