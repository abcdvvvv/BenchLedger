...
import { buildDimensionConfigurations, buildDimensionDefinitions, buildDimensionSelectionIndex, resolveDimensionSelection, type DimensionDefinition, type DimensionValueSelection, type ResolvedDimensionSelection } from "../lib/dimension-selector";
import type { UISettings } from "../lib/dashboard-settings";
import type { LoadedBenchmarkDatabase } from "../lib/types";

export type BenchmarkDimensionSelection = ResolvedDimensionSelection & { dimensions: DimensionDefinition[]; };

const Empty_Dimension_Selection: BenchmarkDimensionSelection = { dimensions: [], varyingDimensionKeys: [], varyingDimension: null, fixedValueSelections: [], validation: { isValid: false, issues: ["Exactly one dimension must be Varying. Currently: 0."] }, configurationKeys: [], points: [], pointKeyByConfigurationKey: new Map() };
...
function persistedValueSelections(current: readonly DimensionValueSelection[], resolved: ResolvedDimensionSelection, dimensions: readonly DimensionDefinition[]): DimensionValueSelection[] {
  ...
  for (const selection of resolved.fixedValueSelections) selections.set(selection.dimension.key, selection.rememberedValueKeys);
...
export function useBenchmarkDimensionSelector(database: LoadedBenchmarkDatabase | null, settings: UISettings, setSetting: <K extends keyof UISettings>(key: K, value: UISettings[K]) => void): BenchmarkDimensionSelection {
  ...
  return database ? { ...resolved, dimensions } : Empty_Dimension_Selection;
}
